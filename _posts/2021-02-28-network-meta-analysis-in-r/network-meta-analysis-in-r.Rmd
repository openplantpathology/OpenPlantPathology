...
description: |
  Meta-analysis has gained popularity in Plant Pathology. Among the several statistical methods, Network Meta-Analysis has been thoroughly presented to plant pathologists in a recent paper. In this post, I reproduce the analysis of this paper which was conducted using SAS. Although the results are not identical, they are very similar. 
author:
...
A recent survey (Del Ponte, unpublished) showed more than 50 meta-analysis articles published using plant disease-related data, mainly to test treatment effects on disease severity and yield, but also their relationship. Although this is VERY small number of studies compared with other fields such as medicine and psychology fields, there is a trend towards increasing its use in our field.

Specifically, multiple treatment comparison (e.g. fungicides and biocontrol agents) is usually of interest and these treatments are all compared with a common treatment, the untreated check, with their effects (relative control) thus being correlated.

Hence, this post is for those who have interest in learning how to fit network meta-analysis models, in particular a frequentist and arm-based approach (as opposed to contrast-based) using open source tools, as alternative to SAS and other commercial software.

Here, I reproduce the analysis in the review paper where the authors made available all the data (wheat yield response to fungicides) and the codes using SAS @madden2016, using one meta-analysis-specific packages in R. This allowed me to fit the same model, the arm-based or unconditional, fitted in the original paper (in this approach the effect sizes are directly the treatment means and not the difference or ration as traditionally performed in meta-analysis).

I will reproduce some, not all, of the arm-based models presented in the original paper using a meta-analysis-specific package called `metafor`. I will explain the details of how to prepare the data, visualize the responses, fit the models and compare results with the original paper.
...
library(tidyverse) # load several useful pkg
...
Let's replace the names of the levels of the treatment factor from number to the fungicide treatment acronym.
...
In the original paper, the authors compared three different variance-covariance matrix for a random-effects model: compound symmetry (CS), heterogeneous CS (HCS) and unstructured (UN). These can be informed directly in the `rma.mv` function.
...
We use the `rma.mv` function for fitting mixed effects model for multivariate and multi-treatment situations. We enter the effect of interest, which is `yield` (means of the treatment of interest across replicates within the same trial) and the sampling variance `varyld` (e.g. the residual variance obtained from the fit of an anova like model). Our moderators will be the additive effect of treatment `trt`. The `method` is the maximum likelihood. The `random` component will be treatment and trial, meaning a random intercept and slope for each treatment. The `struct` is CS and the `data` is wheat.
...
Let's see the summary results of the model. We skip to model results where we can check the estimates for each treatment which are the differences to the intercept, purposely defined as the untreated check. We can see that MET (0.436 ton) and PROP (0.20 ton) provided the highest and lowest numerical yield return. We should also look at the 95% confidence intervals.
...
And the results. Compare with the previous one.
...
We can extract the mean estimate and the respective standard error from each of the models namely R1 (CS), R2(HCS) and R3(UN) as in the original paper.
...
[Source of the figure](https://apsjournals.apsnet.org/doi/10.1094/PHYTO-12-15-0342-RVW)

As in the original paper, we will compare all three models based on the LRT test. The full model uses the UN variance-covariance matrix. Agreeing with the original study, the UN provided a better fit to the data and had the lowest AIC.
...
We can then make a forest plot with treatments ordered from highest to lowest yield difference. The size of the dots is proportional to the inverse of the standard error. 
...
table1 %>% 
  ...
  ylim(0.1,0.6)+
  labs(y = "Yield difference (ton/ha)",
       x = "",
...
Using the best model, we can perform a head-to-head comparison based on the network model results. We can do it using `multcomp` package.
...
And we can also plot the results and more easily check which differences are statistically significant (not embracing zero).
...
In the original paper, two models were expanded to include the effect of wheat class (S = Spring wheat and W = Winter wheat) as moderator. Let's fit the CS model as in the paper and add this moderator in interaction with the treatment. The rest is the same.
...
We fitted three network models to the means of wheat yields under the effect of fungicide treatments. These models had different variance-covariance matrix. Results of these three models and, of the one expanded to include the effect of wheat class, are not identical to those obtained in @madden2016, but they are all very similar.
