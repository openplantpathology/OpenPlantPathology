...
It can be even more complicated and laborious if the manual tweaking is extensive. In brief, it is neither time-effective nor reproducible.
...
I used it until I discovered another game changer, the [{patchwork}](https://patchwork.data-imaginist.com/index.html) package. Both packages are great options and it is really a matter of preference.
...
I will focus mainly on the specifics of how to use the patchwork package to produce the most common type of figures and save them in the PNG format.
...
No [iris](https://www.rdocumentation.org/packages/datasets/versions/3.6.2/topics/iris) or [penguins](https://allisonhorst.github.io/palmerpenguins/index.html) allowed here!
...
As we did for the first plots, we need to assign each plot to a ggplot object before we can produce the multi-panel figure.
...
Is that it?
Yes!
Just load the package and the `|` character will do the magic when connecting the two {ggplot2} objects.
But there is more, actually much more if you look at the documentation of the package.
Let's keep exploring a few cool features for our nice coffee rust data set. Now we switch to the `/` character to see one plot on top of another.
...
By default, each plot occupies the same space in the panel. Sometimes we want them to occupy different spaces, right...
...
Let's load an image using the `draw_image()` function of {cowplot} (which requires the {magick} package to work).
