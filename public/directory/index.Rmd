...
old_locations <- load("./data/lonlat.Rds")

update_location <-
  paste(members$institution,
  members$location,
  members$state,
  members$country,
  sep = ", ")

new_locations <- update_location %notin% location

location <- rbind(members$lon, members$lat, geocode(new_locations))

members <- data.frame(members, location)

# save the location data to check against for next run
save(members, file = "./data/member_lonlat.Rds")
